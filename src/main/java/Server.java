...
public class Server {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ServerSocket serverSocket = new ServerSocket(serverPort);  // порт может быть например 8080
        Socket socket = null;
        ...
        while (true) {
            ...
            String[] output = headerTempData.split(" "); //TODO Почему делим по пробелу?
            String headerData = output[1];


            PrintWriter out = new PrintWriter(socket.getOutputStream()); // тут уже мы отсюда передаем туда ответы
            out.println("HTTP/1.1 20local0 OK");   //TODO Что за 20local0?
    ...
    //Вроде считываем побайтово текст запроса
    public static String getHeaderToArray(InputStream inputStream) {
        ...
        try {
            int c;
            while ((c = reader.read()) != -1) {  //-1 если до конца дошли
